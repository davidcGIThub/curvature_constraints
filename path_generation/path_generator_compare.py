...
class PathGenerator:
    ...
    def __create_optimization_variables(self, waypoints, initial_control_points=None):
        ...
        if self._curvature_method == "control_point_derivatives_rotate":
            rotation_angles = self.__get_initial_rotation_angles(initial_control_points, initial_scale_factor)
            optimization_variables = np.concatenate((optimization_variables, rotation_angles))
    ...
    def __create_objective_variable_bounds(self):
        num_extra_cols = 3
        if self._curvature_method == "control_point_derivatives_rotate":
            num_extra_cols += (self._num_control_points - self._order)
        ...
        if self._curvature_method == "control_point_derivatives_rotate":
            lower_bounds[self._num_control_points*self._dimension+3:] = -np.inf
            upper_bounds[self._num_control_points*self._dimension+3:] = np.inf
    ...
    def __get_rotation_angles(self, variables):
        rotation_angles = variables[self._num_control_points*self._dimension+3:]
    ...
    def __create_waypoint_velocity_constraint(self, velocities):
        def velocity_constraint_function(variables):
            ...
            if self._curvature_method == "constrain_max_acceleration_and_min_velocity":
                start_velocity = (control_points[:,2] - control_points[:,0])/(2*start_waypoint_scalar)
                end_velocity = (control_points[:,-1] - control_points[:,-3])/(2*start_waypoint_scalar)
            else:
                start_velocity = end_waypoint_scalar*(control_points[:,2] - control_points[:,0])/(2)
                end_velocity = end_waypoint_scalar*(control_points[:,-1] - control_points[:,-3])/(2)
    ...
    def __create_curvature_constraint(self, max_curvature):
        def curvature_constraint_function(variables):
            control_points, scale_factor =  self.__get_control_points_and_scale_factor(variables)
            rotation_angles = self.__get_rotation_angles(variables)
            max_curvature_of_spline_intervals = self.__get_max_curvature_of_each_spline_interval(control_points, rotation_angles)
            largest_curvature = np.max(max_curvature_of_spline_intervals)
            constraint = largest_curvature - max_curvature
            return constraint
            # constraints = max_curvature_of_spline_intervals - max_curvature
            # return constraints
        lower_bound = -np.inf
        upper_bound = 0
    ...
    def __get_max_curvature_of_each_spline_interval(self,control_points, rotation_angles):
        ...
        for i in range(num_intervals):
            ...
            if self._curvature_method == "roots_of_curvature_derivative":
                ...
                max_curvatures[i] = get_control_point_curvature_bound(control_points_per_interval,self._order,method="mdm", rotation_angle = None)
            elif self._curvature_method == "control_point_derivatives_rotate":
                rotation_angle = rotation_angles[i]
                max_curvatures[i] = get_control_point_curvature_bound(control_points_per_interval,self._order,method="rotate", rotation_angle = rotation_angle)
    ...
    def __get_initial_rotation_angles(self, control_points, scale_factor):
        num_intervals = self._num_control_points - self._order
        rotation_angles = np.zeros(num_intervals)
        for i in range(num_intervals):
            interval_cont_pts = control_points[:,i:i+self._order+1]
            psi = self.__get_rotation_angle(interval_cont_pts, scale_factor)
            rotation_angles[i] = psi
        return rotation_angles
    
    def __get_rotation_angle(self, control_points, scale_factor):
