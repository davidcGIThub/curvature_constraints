...
TEST(ThirdOrderCurvatureTest, CrossCoeficients2D)
{
    ...
    control_points << 9, 4, 8, 6,
                      1, 5, 5, 5;
    Eigen::Vector4d coeficient_array = ThirdOrderCurvatureEvaluator::get_2D_cross_coefficients(control_points);
...
TEST(ThirdOrderCurvatureTest, CrossCoeficients3D)
{
    ...
    control_points << 2, 4, 4, 2,
                      1, 2, 8, 6,
                      2, 3, 5, 0;
    Eigen::Vector4d coeficient_array = ThirdOrderCurvatureEvaluator::get_3D_cross_coefficients(control_points);
    double true_c_3 = coeficient_array(3);
    double true_c_2 = coeficient_array(2);
    double true_c_1 = coeficient_array(1);
    double true_c_0 = coeficient_array(0);
...
TEST(ThirdOrderCurvatureTest, MatrixRetrieval)
{
    ...
    true_M << -0.16666667,  0.5, -0.5, 0.16666667,
        ...
        0.16666667,           0,    0, 0;
    Eigen::Matrix<double, 4,4> M = ThirdOrderCurvatureEvaluator::get_third_order_M_matrix();
...
TEST(ThirdOrderCurvatureTest, DerivativeVectorRetrieval)
{
    ...
    Eigen::Vector4d dt_vector = ThirdOrderCurvatureEvaluator::get_third_order_T_derivative_vector(t,scale_factor);
    float tolerance = 0.00000001;
...
TEST(ThirdOrderCurvatureTest, SecondDerivativeVectorRetrieval)
{
    ...
    Eigen::Vector4d ddt_vector = ThirdOrderCurvatureEvaluator::get_third_order_T_second_derivative_vector(t);
    float tolerance = 0.00000001;
...
TEST(ThirdOrderCurvatureTest, DerivativeVectorRetrievalError)
{
    ...
    {
        double t = -0.4;
        ThirdOrderCurvatureEvaluator::get_third_order_T_derivative_vector(t,scale_factor);
    }
...
TEST(ThirdOrderCurvatureTest, SecondDerivativeVectorRetrievalError)
{
    ...
    {
        double t = 1.3;
        ThirdOrderCurvatureEvaluator::get_third_order_T_second_derivative_vector(t);
    }
    catch(std::invalid_argument const& e)
    {
        EXPECT_STREQ("t value should be between 0 and 1", e.what());
...
TEST(ThirdOrderCurvatureTest, VelocityMagnitude)
{
    ...
    double velocity = ThirdOrderCurvatureEvaluator::calculate_velocity_magnitude(time_t, control_points,scale_factor);
...
TEST(ThirdOrderCurvatureTest, AccelerationMagnitude)
{
    ...
    double acceleration = ThirdOrderCurvatureEvaluator::calculate_acceleration_magnitude(time_t, control_points);
...
TEST(ThirdOrderCurvatureTest, CrossTermMagnitude)
{
    ...
    double cross_term = ThirdOrderCurvatureEvaluator::calculate_cross_term_magnitude(time_t, control_points,scale_factor);
...
TEST(ThirdOrderCurvatureTest, RandomNormalSpline)
{
    ...
    double curvature_bound = ThirdOrderCurvatureEvaluator::evaluate_interval_curvature(control_points);
    double tolerance = 0.00000000001;
...
TEST(ThirdOrderCurvatureTest, SplineCurvatureBound)
{
    ...
    double max_curvature = ThirdOrderCurvatureEvaluator::find_spline_curvature_bound(control_points, num_control_points);
...
TEST(ThirdOrderCurvatureTest, MinVelocityOfSpline)
{
    ...
    double control_points[] = {   -0.89402549, -0.05285741,  1.10545513,  2.47300498,
         3.79358126,  4.76115495,  5.11942253,  4.76115495, -0.10547684,  0.05273842,
        -0.10547684, -0.47275804, -0.79306865, -0.76080139, -0.11946946,  1.23867924};
    double min_velocity = ThirdOrderCurvatureEvaluator::find_min_velocity_of_spline(control_points, num_control_points,scale_factor);
    double tolerance = 0.0000001;
    EXPECT_NEAR(true_min_velocity, min_velocity, tolerance);
}
