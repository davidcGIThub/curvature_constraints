...
class WaypointConstraints(object):
    ...
    def curvature_at_waypoints_constraints(self, cont_pts, scale_factor, desired_curvatures):
        num_cont_pts = np.shape(cont_pts)[1]
        cont_pts_array = cont_pts.flatten().astype('float64')
        des_curvature_array = desired_curvatures.flatten().astype('float64')
        if self._dimension == 2:
            objective = lib.curvature_at_waypoints_constraints_2(self.obj, cont_pts_array, num_cont_pts, scale_factor, des_curvature_array)
        else: # value = 3
            objective = lib.curvature_at_waypoints_constraints_3(self.obj, cont_pts_array, num_cont_pts, scale_factor, des_curvature_array)
        return objective
...
vel_at_waypoints_constraints = const_func.velocity_at_waypoints_constraints(control_points, scale_factor, desired_velocities)
...
curvature_at_waypoints_constraints = const_func_3.curvature_at_waypoints_constraints(control_points, scale_factor, desired_curvatures)
