...
TEST(ObstacleConstraintTests, CenterInMinvoHull)
{
    ...
    float distance = obst_const.getObstacleDistanceToSpline(control_points, num_spline_points, sphere_radius, sphere_center);
    float true_distance = 0.9999843316663152;
...
TEST(ObstacleConstraintTests, RadiusInMinvoHull)
{
    const int D{2};
    ObstacleConstraints<D> obst_const{};
    float sphere_center[] = {4.59242023, 2.80862623};
    float sphere_radius = 1; 
    const int num_spline_points = 10;
    float control_points[] = {3.51490868, 9.93890562, 3.18371347, 4.82774563, 1.53139681, 0.38652503, 6.0471705,  0.16208627, 4.40754293, 3.47274037,
        3.91808237, 8.35404157, 1.90443825, 8.44816466, 4.16849762, 3.4327385, 1.76263076, 5.31268297, 2.22008256, 2.8775095};
    float distance = obst_const.getObstacleDistanceToSpline(control_points, num_spline_points, sphere_radius, sphere_center);
    float true_distance = 0.6514747211179606;
...
TEST(ObstacleConstraintTests, RadiusInBsplineHull)
{
    ...
    float distance = obst_const.getObstacleDistanceToSpline(control_points, num_spline_points, sphere_radius, sphere_center);
    float true_distance = -1.7486032163129486;
    float tolerance = 0.000001;
    EXPECT_NEAR(true_distance, distance, tolerance);
}
...
TEST(ObstacleConstraintTests, PointInBsplineHull)
{
    const int D{2};
    ObstacleConstraints<D> obst_const{};
    float sphere_center[] = {2.09598594 , 5.31302225};
    float sphere_radius = 1; 
    const int num_spline_points = 10;
    float control_points[] = {1.71441477, 3.6139517,  5.30094221, 8.37323056, 9.6235351,  3.90841661, 1.99899417, 8.67304047, 5.21592068, 2.55523126,
        8.09595843, 1.83482911, 7.77011536, 6.37508643, 5.37388355, 0.48746415, 4.62696183, 1.95456998, 7.39982416, 9.53329381};
    float distance = obst_const.getObstacleDistanceToSpline(control_points, num_spline_points, sphere_radius, sphere_center);
    float true_distance = -1.0435314502170652;
    float tolerance = 0.000001;
    EXPECT_NEAR(true_distance, distance, tolerance);
}
...
TEST(ObstacleConstraintTests, OutsideBsplineHull)
{
    const int D{2};
    ObstacleConstraints<D> obst_const{};
    float sphere_center[] = {-1.8820112, -0.61966279};
    float sphere_radius = 1; 
    const int num_spline_points = 10;
    float control_points[] = {5.40876234, 3.59385918, 8.64430584, 1.54778121, 3.28521738, 6.45670928, 2.4775997,  2.13578183, 5.73122222, 4.73792694,
        6.48052042, 6.70371021, 1.27331269, 0.54655433, 1.03721891, 0.41984464, 2.40628853, 3.16038393, 8.05965801, 9.55043722};
    float distance = obst_const.getObstacleDistanceToSpline(control_points, num_spline_points, sphere_radius, sphere_center);
    float true_distance = -8.219382383246899;
    float tolerance = 0.000001;
    EXPECT_NEAR(true_distance, distance, tolerance);
}
...
TEST(ObstacleConstraintTests, ThreeDimensionTest)
{
    const int D{3};
    ObstacleConstraints<D> obst_const{};
    float sphere_center[] = {1.97355228, 4.03641385, 2.56398793};
    float sphere_radius = 1; 
    const int num_spline_points = 10;
    float control_points[] = {0.76042949, 3.42851596, 7.12733203, 5.40578535, 6.19092292, 4.19655381, 7.35779894, 4.71393053, 8.11393068, 1.14161887,
    ...
    float distance = obst_const.getObstacleDistanceToSpline(control_points, num_spline_points, sphere_radius, sphere_center);
    float true_distance = -1.1481243240992596;
    float tolerance = 0.000001;
    EXPECT_NEAR(true_distance, distance, tolerance);
}
...
TEST(ObstacleConstraintTests, TestMultipleObstacles)
{
    ...
    float* distances = obst_const.getObstacleDistancesToSpline(control_points, num_spline_points,
                                          obstacle_radii, obstacle_centers, num_obstacles);
    float true_distance_1 = 2.6063083134151492;
    float true_distance_2 = -7.605461310820617;
    float true_distance_3 = -1.4706446781371505;
    float tolerance = 0.000001;
