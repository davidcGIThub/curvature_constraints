...
TEST(DerivativeTest, MatrixRetrieval)
{
    ...
    Eigen::Matrix<float, 4,4> true_M;
    true_M << -0.16666667,  0.5, -0.5, 0.16666667,
               0.5,          -1,    0, 0.66666667,
              -0.5,         0.5,  0.5, 0.16666667,
        0.16666667,           0,    0, 0;
    Eigen::Matrix<float, 4,4> M = c_eval.get_third_order_M_matrix();
...
TEST(DerivativeTest, DerivativeVectorRetrieval)
{
    ...
    Eigen::Vector4f true_dt_vector;
    true_dt_vector << 0.36757914, 0.70007536, 1, 0;
    float t = 0.35003768191677603;
    Eigen::Vector4f dt_vector = c_eval.get_third_order_T_derivative_vector(t,scale_factor);
...
TEST(DerivativeTest, SecondDerivativeVectorRetrieval)
{
    float scale_factor = 1;
    DerivativeEvaluator<2> c_eval{};
    Eigen::Vector4f true_ddt_vector;
    float t = 0.5199907026925293;
    true_ddt_vector << 3.11994422, 2, 0, 0;
    Eigen::Vector4f ddt_vector = c_eval.get_third_order_T_second_derivative_vector(t,scale_factor);
...
TEST(DerivativeTest, VelocityMagnitude)
{
    DerivativeEvaluator<2> c_eval{};
    float scale_factor = 1;
    float true_velocity = 2.8230311463199973;
    Eigen::Matrix<float, 2,4> control_points;
    control_points << 4, 7, 4, 4,
                      3, 5, 8, 4;
    float time_t = 0.35768880134338066;
    float velocity = c_eval.calculate_velocity_magnitude(time_t, control_points,scale_factor);
...
{
    DerivativeEvaluator<2> c_eval{};
    float true_acceleration = 2.2568129685516602;
    Eigen::Matrix<float, 2,4> control_points;
    control_points << 8, 3, 6, 4,
                      6, 5, 7, 3;
    float time_t = 0.4696969696969697;
    float scale_factor = 1;
    float acceleration = c_eval.calculate_acceleration_magnitude(time_t, control_points, scale_factor);
    float tolerance = 0.000001;
...
TEST(DerivativeTest, MinVelocity)
{
    DerivativeEvaluator<2> c_eval{};
    float scale_factor = 1;
    float true_min_velocity = 0.12523779030065027;
    Eigen::Matrix<float, 2,4> control_points;
    control_points << 0.1,  3.2,  2.01, 1.1,
                      0.01, 2.98, 2.1,  0.99; 
    std::array<float,2> min_velocity_and_time = c_eval.find_min_velocity_and_time(control_points,scale_factor);
...
{
    DerivativeEvaluator<2> c_eval{};
    float true_max_acceleration = 9.055385138137417;
    Eigen::Matrix<float, 2,4> control_points;
    control_points << 7, 4, 2, 0,
                      3, 1, 8, 7;
    float scale_factor = 1;
    std::array<float,2> max_acceleration_and_time = c_eval.find_max_acceleration_and_time(control_points, scale_factor);
...
TEST(DerivativeTest, MinVelocityOfSpline)
{
    DerivativeEvaluator<2> c_eval{};
    float scale_factor = 1;
    float true_min_velocity = 0.7068769432442363;
    ...
    float min_velocity = c_eval.find_min_velocity_of_spline(control_points, num_control_points,scale_factor);
    float tolerance = 0.00001;
    EXPECT_NEAR(true_min_velocity, min_velocity, tolerance);
}
...
TEST(DerivativeTest, MaxAccelerationOfSpline)
{
    DerivativeEvaluator<2> c_eval{};
    float scale_factor = 1;
    float true_max_acceleration = 1.0135328890911517;
    int num_control_points = 8;
    float control_points[] = {   -0.89402549, -0.05285741,  1.10545513,  2.47300498,
         3.79358126,  4.76115495,  5.11942253,  4.76115495, -0.10547684,  0.05273842,
        -0.10547684, -0.47275804, -0.79306865, -0.76080139, -0.11946946,  1.23867924};
    float max_acceleration = c_eval.find_max_acceleration_of_spline(control_points, num_control_points,scale_factor);
    float tolerance = 0.00001;
...
TEST(DerivativeTest, MinVelocityOfSpline3D)
{
    ...
    float scale_factor = 1;
    float true_min_velocity = 1.5;
    int num_control_points = 8;
    float control_points[] = {3, 0, 5, 1, 9, 6, 6, 4,
                                6, 2, 6, 8, 0, 8, 0, 6,
                                0, 0, 8, 0, 4, 7, 1, 8};
    float min_velocity = c_eval.find_min_velocity_of_spline(control_points, num_control_points,scale_factor);
    float tolerance = 0.00001;
...
TEST(DerivativeTest, MaxAccelerationOfSpline3D)
{
    DerivativeEvaluator<3> c_eval{};
    float scale_factor = 1;
    float true_max_acceleration = 19.697715603592208;
    int num_control_points = 8;
    float control_points[] = {3, 0, 5, 1, 9, 6, 6, 4,
                                6, 2, 6, 8, 0, 8, 0, 6,
                                0, 0, 8, 0, 4, 7, 1, 8};
